...
)
{
	...
	for (block = 0; block < nblocks; block++)
	{
		...
		if (nk <= 3)
		{
			...
			lnz1 = (double)nk * (nk + 1) / 2;
			flops1 = (double)nk * (nk - 1) / 2 + (double)(nk - 1) * nk * (2 * nk - 1) / 6;
			ok = TRUE;
		...
		else if (ordering == 0)
		{
			...
			lnz1 = (double)((Int)(amd_Info[AMD_LNZ]) + nk);
		...
		else
		{
			...
			lnz1 = (double)((Common->user_order) (nk, Cp, Ci, Pblk, Common));
			flops1 = EMPTY;
			ok = (lnz1 != 0);
		}
